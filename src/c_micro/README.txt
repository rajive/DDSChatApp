...
can be invoked manually, with an example command like this:
    ...
    ${RTIMEHOME}/rtiddsgen/scripts/rtiddsgen -micro -language C Chat.idl

The generated source files are Chat.c, ChatSupport.c, and 
ChatPlugin.c. Associated header files are also generated.
 
The DataWriter and DataReader of the type are managed in Chat_publisher.c
and Chat_subscriber.c, respectively. The DomainParticipant of each is 
managed in ChatApplication.c
...
Darwin
------
cmake [-DCMAKE_BUILD_TYPE=<Debug|Release>] -G "Unix Makefiles" -B./<your build directory> -H. -DRTIME_TARGET_NAME=x64Darwin17clang9.0 -DPLATFORM_LIBS="dl;m;pthread"
cmake --build ./<your build directory> [--config <Debug|Release>]
